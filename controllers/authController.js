...
exports.registerCompanyUser = async (req, res, next) => {
  ...
  if (!["image/jpeg", "image/png"].includes(logoData.mimetype)) {
  ...
  if (logoData.size > 1 * 1024 * 216) {
    return res.status(400).json({
      success: false,
      message: "Image size should not exceed 216KB",
    });
  }
  ...
  try {
    ...
    // Add company details linked to the user
    ...
    for (let i = 0; i < documentTypes.length; i++) {
      ...
      if (file.size > 1 * 1024 * 1024) {
        throw new Error("Document size must be less than 1MB");
      }
...
exports.getManagers = async (req, res, next) => {
  try {
    const managers = await User.findAll({
      where: {
        role: {
          [Op.in]: ["productManager", "orderManager"],
        },
